  ...
  Status InternalAdd(const Slice& key, const Slice& value,
    ...
    if (entry_type == kEntryMerge || entry_type == kEntryPut ||
      ...
      if (entry_type == kEntryMergeIndex || entry_type == kEntryValueIndex) {
        // Strip the optional chunk-id trailer so that TTL extractors
        // receive exactly the raw meta emitted by the user-provided
        // ValueExtractor, regardless of whether the chunk-aware
        // value-index format is enabled for this CF. For legacy
        // value-indices this call is bit-for-bit identical to
        // DecodeValueMeta().
        value_or_meta = SeparateHelper::DecodeValueMetaStripChunk(value);
      }
