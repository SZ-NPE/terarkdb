...
class VersionStorageInfo {
 public:
  ...
  // captures the union of data-block references coming from every SST
  // in the current version (`live_block_bitmap`) plus a few derived
  ...
  // `BlobBlockBitmap` produced for legacy or block-unaware paths), the
  // blob's `bitmap_available` flag is sticky-cleared. In that regime
  // GC must fall back to the legacy GetKey()-based path.
  //
  // `total_block_count` is preferably the vSST data block count taken
  // from its TableProperties; when unknown it falls back to
  // bitmap.num_bits() (which cannot be used for physical block skip).
  struct BlobLiveBlockInfo {
    BlobBlockBitmap live_block_bitmap;
    bool bitmap_available = true;
    uint64_t live_block_count = 0;
    uint64_t total_block_count = 0;
    uint64_t live_block_bytes = 0;
    uint64_t dead_block_bytes = 0;
    double dead_block_ratio = 0.0;
  };

  using BlobLiveBlockMap = std::unordered_map<uint64_t, BlobLiveBlockInfo>;

  // Aggregate per-dependence block bitmaps coming from every SST in
  // the current version into per-blob-file live block views. The
  // method is idempotent: it always wipes the previous state before
  // recomputing, so it is safe to invoke multiple times on the same
  // VersionStorageInfo (e.g. after an option change).
  void AggregateBlobLiveBlockBitmaps();

  // Wiring helper: discard any previously-aggregated
  // blob_live_block_info_ without running the aggregation pass.
  // Used by Version::PrepareApply() when the CF feature toggle
  // (enable_blob_block_bitmap) is off, so that GC consumers
  // consistently observe "bitmap unavailable" instead of a stale view
  // produced under a previous option regime.
  void ClearBlobLiveBlockInfo() { blob_live_block_info_.clear(); }

  const BlobLiveBlockMap& blob_live_block_info() const {
  ...
  // -----------------------------------------------------------------
  // GC fast-path liveness query.
  //
  // Three-state result:
  //   kLive    : the aggregated bitmap says the block has at least one
  //              SST reference in the current version. Callers still
  //              need to do per-record validation (a block can contain
  //              both live and dead records).
  //   kDead    : the aggregated bitmap is available AND no SST in the
  //              current version references this block. GC may drop
  //              every record in this block without a GetKey() lookup.
  ...
  // -----------------------------------------------------------------
  enum class BlobBlockLiveness : uint8_t { kLive, kDead, kUnknown };
  ...
  BlobBlockLiveness IsBlockLive(uint64_t blob_file_number,
    ...
    const BlobLiveBlockInfo& info = it->second;
    if (!info.bitmap_available) {
      return BlobBlockLiveness::kUnknown;
    }
    return info.live_block_bitmap.Test(block_id)
               ? BlobBlockLiveness::kLive
               : BlobBlockLiveness::kDead;
  }

  // Blob-level fast-path gate. Returns true iff every block in the
  // blob is provably dead under the aggregated bitmap. When this
  // holds, GC may drop every record in the blob without any GetKey()
  ...
  bool IsBlobEntirelyDead(uint64_t blob_file_number) const {
    ...
    const BlobLiveBlockInfo& info = it->second;
    return info.bitmap_available && info.live_block_count == 0;
  }
 ...
 private:
  ...
  // Per-blob-file aggregated live-block view. Filled by
  // AggregateBlobLiveBlockBitmaps(); empty until then. Internal-only;
  // not part of the on-disk manifest representation.
  BlobLiveBlockMap blob_live_block_info_;
