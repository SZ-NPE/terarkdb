...
#ifdef USE_LOG_STORE
#include "bytestore_fs/bytestore_env.h"

namespace TERARKDB_NAMESPACE {

class BytestoreSequentialFile : public SequentialFile {
 public:
  explicit BytestoreSequentialFile(std::unique_ptr<FSSequentialFile>&& target)
...
class BytestoreRandomAccessFile : public RandomAccessFile {
 public:
  explicit BytestoreRandomAccessFile(
...
class BytestoreWritableFile : public WritableFile {
 public:
  explicit BytestoreWritableFile(std::unique_ptr<FSWritableFile>&& target)
      : target_(std::move(target)) {}
...
class BytestoreDirectory : public Directory {
 public:
  explicit BytestoreDirectory(std::unique_ptr<FSDirectory>&& target)
      : target_(std::move(target)) {}
...
class BytestoreEnv : public EnvWrapper {
 public:
  // Initialize an EnvWrapper that delegates all calls to *t
  explicit BytestoreEnv(Env* t) : EnvWrapper(t), target_(t) {}

  ~BytestoreEnv() override {}

  Status InitBytestore(const std::string& uri, const std::string& sdk_log_dir,
                       bool enable_rdma) {
    return bytestore_fs::NewBytestoreFileSystem(uri, sdk_log_dir, enable_rdma,
                                                &fs_);
  }
  ...
  Status NewSequentialFile(const std::string& f,
    ...
    if (s.ok()) {
      r->reset(new BytestoreSequentialFile(std::move(file)));
    }
    return s;
  }
  ...
  Status NewRandomAccessFile(const std::string& f,
    ...
    if (s.ok()) {
      r->reset(new BytestoreRandomAccessFile(std::move(file)));
    }
    return s;
  }
  ...
  Status NewWritableFile(const std::string& f, std::unique_ptr<WritableFile>* r,
    ...
    if (s.ok()) {
      r->reset(new BytestoreWritableFile(std::move(file)));
    }
    return s;
  }
  ...
  Status NewDirectory(const std::string& name,
    ...
    if (s.ok()) {
      result->reset(new BytestoreDirectory(std::move(dir)));
    }
    return s;
  }
  ...
  bool find_bytestore_prefix(const std::string& dir) {
    return dir.rfind(bytestore_fs::BytestoreFileSystem::kBytestoreBlobScheme,
                     0) == 0 ||
           dir.rfind(
               bytestore_fs::BytestoreFileSystem::kBytestoreLocalBlobScheme,
               0) == 0;
  }

  Status GetChildren(const std::string& dir,
                     std::vector<std::string>* r) override {
    if (find_bytestore_prefix(dir)) {
...
Status NewBytestoreEnv(Env** result, const std::string& uri, bool enable_rdma) {
  auto env = new BytestoreEnv(Env::Default());
  std::string sdk_log_dir = "./bytestore_sdk";
  Status s = env->CreateDirIfMissing(sdk_log_dir);
  if (s.ok()) {
    s = env->InitBytestore(uri, sdk_log_dir, enable_rdma);
  }
  *result = s.ok() ? env : nullptr;
  return s;
}
...
Status NewBytestoreEnv(Env** result, const std::string& uri, bool enable_rdma) {
  *result = nullptr;
  return Status::NotSupported("ByteStoreEnv is not implemented.");
}
}  // namespace TERARKDB_NAMESPACE
#endif
